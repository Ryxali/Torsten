...
public class PaletteStore {
	...
	/**
	 * Creates a new PaletteStore using the current user file as a reference as
	 * to what palettes to feature.
	 */
	 ...
	 * 
	 * @param index
	 *            the index at the Palette array
	 * @param palette
	 *            the new palette
	 */
	 ...
	 * 
	 * @param index
	 *            the index in the palettes array
	...
	public boolean contains(int pointX, int pointY, int screenWidth,
			int screenHeight) {
		if (screenWidth - getActivePalette().getWidth() <= pointX
				&& pointX <= screenWidth) {
			if (getActivePalette().getHeight() + Palette.Y_POS >= pointY
					&& pointY >= 0) {
				return true;
			}
		}
	...
	/**
	 * Draws the currently active pallette onto the screen as well as all of the
	 * paletteSwitchButtons above it
	 * 
	 * @param g
	...
	public void draw(Graphics g, int screenWidth, int screenHeight, Input input) {
		...
		g.fillRect(screenWidth - getActivePalette().getWidth(), getY(),
				getActivePalette().getWidth(), 50);
		for (int i = 0; i < pButtons.length; i++) {
			pButtons[i].draw(g, screenWidth - getActivePalette().getWidth()
					+ getActivePalette().getWidth() / pButtons.length * i,
					Palette.Y_POS - HEIGHT, getActivePalette().getWidth()
							/ pButtons.length, HEIGHT, screenWidth,
					screenHeight, input);
			// g.setColor(Color.black);
			// g.drawRect(Palette.X_POS,
			// Palette.Y_POS-50,getActivePalette().getWidth()/palettes.length*i,
			// 50);
		}
		getActivePalette().draw(g, screenWidth - getActivePalette().getWidth(),
				Palette.Y_POS, screenWidth, screenHeight, input);
	}

	public void update(Input input) {
		for (int i = 0; i < pButtons.length; i++) {
			// pButtons[i].buttonStateCheck(input,
			// Palette.X_POS+getActivePalette().getWidth()/pButtons.length*i,
			// Palette.Y_POS-50, getActivePalette().getWidth()/pButtons.length);
