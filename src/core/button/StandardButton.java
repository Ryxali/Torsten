...
public class StandardButton extends Button {
	 ...
	 *            the image displayed when this button is in its pressed state.
	 */
	public StandardButton(DefaultImage idleImg, DefaultImage hoverImg,
	...
	/*
	 * public StandardButton(ImageStore idleImg, ImageStore hoverImg, ImageStore
	 * pressedImg) { super(0, 0); this.idleImg = idleImg.getImage();
	 * this.hoverImg = hoverImg.getImage(); this.pressedImg =
	 * pressedImg.getImage(); }
	 */
	/**
	 * Creates a new StandardButton with three images to correspond with its
	 ...
	 *            the image displayed when this button is in its pressed state.
	 */
	public StandardButton(Image idleImg, Image hoverImg, Image pressedImg) {
		super();
	 ...
	 * if (input.isMouseButtonDown(Input.MOUSE_LEFT_BUTTON) && (getState() ==
	 * STATE_HOVER || getState() == STATE_PRESSED)) { setState(STATE_PRESSED);
	 * setClicked(true); } else { if
	 * (!input.isMouseButtonDown(Input.MOUSE_LEFT_BUTTON)) {
	 * setState(STATE_HOVER); } else { setState(STATE_IDLE); } } } else if
	 * (!(input.isMouseButtonDown(Input.MOUSE_LEFT_BUTTON) && getState() ==
	 * STATE_PRESSED)) { setState(STATE_IDLE); setClicked(false); } }
	 */
	...
	public void draw(Graphics g, int x, int y, Input input) {
		update(g, x, y, getProperImage().getWidth(), getProperImage()
				.getHeight(), input);
	 ...
	 * 
	 * @param x
	 *            the x position for this button.
	 * @param y
	 *            the y position for this button.
	 * @param input
	 *            the current user input.
	 * @return
	 */
	protected void buttonStateCheck(int x, int y, Input input) {
		buttonStateCheck(input, x, y, getProperImage().getWidth(),
				getProperImage().getHeight());
	}

	/**
	 * Checks if the given point is within this button's bounds and at the given
	 * position.
	 * 
	 * @param pointX
