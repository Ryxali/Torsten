...
/**
 * A button based on GButton that's drawn with a given text string on it.
 * 
 * @author Niklas L
 * @see core.button.GButton
 * 
 */
public class TextGButton extends GButton {
	...
	public void draw(Graphics g, int x, int y, int width, int height,
		...
		g.drawString(text, x + width / 2 - g.getFont().getWidth(text) / 2, y
				+ height / 2 - g.getFont().getHeight(text) / 2);
	}
}
